...
## `aura_count` (see GameState.aura_count) is how many distinct defenders on
## this unit spawn with a colored aura (AuraDB) - 0 by default.
static func make_defense(rng: RandomNumberGenerator, strength_rating: float, aura_count: int = 0) -> Array[PlayerData]:
	...
	var tagged := {}
	var n := mini(aura_count, d.size())
	# Distinct random indices without Array.shuffle - that reads Godot's own
	# global RNG rather than `rng`, which would break the seeded determinism
	# the tuning harnesses rely on (see random_number's neighboring doc).
	while tagged.size() < n:
		var idx := rng.randi_range(0, d.size() - 1)
		if tagged.has(idx):
			continue
		tagged[idx] = true
